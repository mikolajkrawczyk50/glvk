...
static bool test_convolution(ncnn::VulkanDevice* vkdev, int w, int h, int in_c, int out_c, int kernel, int stride, int pad, bool use_fp16 = false) {
    ...
    ncnn::Layer* conv_gpu = ncnn::create_layer(ncnn::LayerType::Convolution);
    ncnn::Layer* conv_cpu = ncnn::create_layer(ncnn::LayerType::Convolution);
...
static bool test_deconvolution(ncnn::VulkanDevice* vkdev, int w, int h, int in_c, int out_c, int kernel, int stride, int pad, bool use_fp16 = false) {
    ...
    ncnn::Layer* deconv_gpu = ncnn::create_layer(ncnn::LayerType::Deconvolution);
    ncnn::Layer* deconv_cpu = ncnn::create_layer(ncnn::LayerType::Deconvolution);
    ...
    int out_w = (w - 1) * stride + kernel - 2 * pad;
    int out_h = (h - 1) * stride + kernel - 2 * pad;
    deconv_gpu->bottom_shapes.resize(1);
    deconv_gpu->bottom_shapes[0] = ncnn::Mat(w, h, in_c, (void*)0);
    deconv_gpu->top_shapes.resize(1);
    deconv_gpu->top_shapes[0] = ncnn::Mat(out_w, out_h, out_c, (void*)0);
...
static bool test_prelu(ncnn::VulkanDevice* vkdev, int w, int h, int c, bool use_fp16 = false) {
    ...
    ncnn::Layer* prelu_gpu = ncnn::create_layer(ncnn::LayerType::PReLU);
    ncnn::Layer* prelu_cpu = ncnn::create_layer(ncnn::LayerType::PReLU);
...
static bool test_binaryop(ncnn::VulkanDevice* vkdev, int op_type, int w, int h, int c, bool use_fp16 = false) {
    ...
    ncnn::Layer* bin_gpu = ncnn::create_layer(ncnn::LayerType::BinaryOp);
    ncnn::Layer* bin_cpu = ncnn::create_layer(ncnn::LayerType::BinaryOp);
    bin_gpu->vkdev = vkdev;
...
static bool test_interp(ncnn::VulkanDevice* vkdev, int w, int h, int c, bool use_fp16 = false) {
    ...
    ncnn::Layer* interp_gpu = ncnn::create_layer(ncnn::LayerType::Interp);
    ncnn::Layer* interp_cpu = ncnn::create_layer(ncnn::LayerType::Interp);
...
static bool test_concat(ncnn::VulkanDevice* vkdev, int w, int h, int c1, int c2, bool use_fp16 = false) {
    ...
    ncnn::Layer* concat_gpu = ncnn::create_layer(ncnn::LayerType::Concat);
    ncnn::Layer* concat_cpu = ncnn::create_layer(ncnn::LayerType::Concat);
...
static bool test_crop_slice(ncnn::VulkanDevice* vkdev, int w, int h, int total_c, int slice_c_start, int slice_c_len, bool use_fp16 = false) {
    ...
    ncnn::Layer* crop_gpu = ncnn::create_layer(ncnn::LayerType::Crop);
    ncnn::Layer* crop_cpu = ncnn::create_layer(ncnn::LayerType::Crop);
...
static bool test_sigmoid(ncnn::VulkanDevice* vkdev, int w, int h, int c, bool use_fp16 = false) {
    ...
    ncnn::Layer* sig_gpu = ncnn::create_layer(ncnn::LayerType::Sigmoid);
    ncnn::Layer* sig_cpu = ncnn::create_layer(ncnn::LayerType::Sigmoid);
    sig_gpu->vkdev = vkdev;
...
// 8b. PixelShuffle layer test
static bool test_pixelshuffle(ncnn::VulkanDevice* vkdev, int w, int h, int c, int upscale_factor) {
    std::string name = "PixelShuffle(w=" + std::to_string(w) + ", h=" + std::to_string(h) + ", c=" + std::to_string(c) +
                       ", scale=" + std::to_string(upscale_factor) + ")";

    ncnn::Option opt = make_vulkan_opt(vkdev, false);

    ncnn::Layer* ps_gpu = ncnn::create_layer(ncnn::LayerType::PixelShuffle);
    ncnn::Layer* ps_cpu = ncnn::create_layer(ncnn::LayerType::PixelShuffle);
    ps_gpu->vkdev = vkdev;

    ncnn::ParamDict pd;
    pd.set(0, upscale_factor);
    ps_gpu->load_param(pd);
    ps_cpu->load_param(pd);

    ps_gpu->create_pipeline(opt);
    ncnn::Option cpu_opt;
    ps_cpu->create_pipeline(cpu_opt);

    ncnn::Mat in(w, h, c);
    for (int q = 0; q < c; q++) {
        float* p = in.channel(q);
        for (int i = 0; i < w * h; i++) p[i] = (q + 1) * 1.5f + i * 0.05f;
    }
    ...
    ncnn::Mat out_gpu_cpu;
    {
        ...
        ncnn::VkMat in_gpu, out_gpu;
        cmd.record_upload(in, in_gpu, opt);
        ps_gpu->forward(in_gpu, out_gpu, cmd, opt);
        cmd.record_download(out_gpu, out_gpu_cpu, opt);
        cmd.submit_and_wait();
    }

    ncnn::Mat out_cpu;
    ps_cpu->forward(in, out_cpu, cpu_opt);

    bool pass = compare_mats(out_gpu_cpu, out_cpu, name, 1e-2f);

    ps_gpu->destroy_pipeline(opt);
    ps_cpu->destroy_pipeline(cpu_opt);
    delete ps_gpu;
    delete ps_cpu;
...
// 8c. BinaryOp with Scalar test
static bool test_binaryop_scalar(ncnn::VulkanDevice* vkdev, int op_type, float scalar_b, int w, int h, int c) {
    std::string name = "BinaryOp_Scalar(op=" + std::to_string(op_type) + ", b=" + std::to_string(scalar_b) +
    ...
    for (int q = 0; q < c; q++) {
        float* p = in.channel(q);
        for (int i = 0; i < w * h; i++) p[i] = (i + 1) * 0.25f + q;
    }
    ...
    ncnn::Mat out_gpu_cpu;
    {
        ...
        ncnn::VkMat in_gpu, out_gpu;
        cmd.record_upload(in, in_gpu, opt);
        bin_gpu->forward(in_gpu, out_gpu, cmd, opt);
        cmd.record_download(out_gpu, out_gpu_cpu, opt);
        cmd.submit_and_wait();
    }

    ncnn::Mat out_cpu;
    bin_cpu->forward(in, out_cpu, cpu_opt);
...
static int cpu_warp_forward(const std::vector<ncnn::Mat>& bottom_blobs, std::vector<ncnn::Mat>& top_blobs, const ncnn::Option& opt) {
...
static bool test_warp_layer(ncnn::VulkanDevice* vkdev, int w, int h, int c) {
    ...
    cpu_warp_forward(bottom_blobs, top_blobs, cpu_opt);
...
int main() {
    ...
    std::cout << "\n--- 1. Convolution Tests (Non-Square) ---" << std::endl;
    all_passed &= test_convolution(vkdev, 32, 16, 16, 32, 3, 1, 1, false);
    all_passed &= test_convolution(vkdev, 16, 32, 16, 32, 3, 1, 1, false);

    std::cout << "\n--- 4. BinaryOp Tests (Non-Square) ---" << std::endl;
    all_passed &= test_binaryop(vkdev, 0, 32, 16, 8, false); // Add
    all_passed &= test_binaryop(vkdev, 0, 16, 32, 8, false);

    std::cout << "\n--- 5. Interp Tests (Non-Square) ---" << std::endl;
    all_passed &= test_interp(vkdev, 32, 16, 4, false);
    all_passed &= test_interp(vkdev, 16, 32, 4, false);

    std::cout << "\n--- 8b. PixelShuffle Tests (Non-Square) ---" << std::endl;
    all_passed &= test_pixelshuffle(vkdev, 32, 16, 16, 2);
    all_passed &= test_pixelshuffle(vkdev, 16, 32, 16, 2);

    std::cout << "\n--- 9. rife.Warp Layer Tests (Non-Square) ---" << std::endl;
    all_passed &= test_warp_layer(vkdev, 64, 32, 3);
    all_passed &= test_warp_layer(vkdev, 32, 64, 3);
    ...
    all_passed &= test_binaryop_scalar(vkdev, 2, 8.0f, 32, 32, 4); // Scalar Mul
    all_passed &= test_binaryop_scalar(vkdev, 3, 4.0f, 32, 32, 4); // Scalar Div
    ...
    std::cout << "\n--- 8b. PixelShuffle Tests ---" << std::endl;
    all_passed &= test_pixelshuffle(vkdev, 16, 16, 16, 2);
    all_passed &= test_pixelshuffle(vkdev, 32, 32, 24, 2);
